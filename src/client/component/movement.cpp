...
namespace movement
{
	...
	namespace
	{
		...
		*/
		...
		void begin_weapon_change_stub(game::pmove_t* pm, game::Weapon new_weap, bool is_new_alt, bool quick, unsigned int* holdrand)
		{
			...
			{
				should_sprint_stall = ((pm->cmd.buttons & game::BUTTON_SPRINT) != 0 || pm->ps->sprintState.lastSprintStart > pm->ps->sprintState.lastSprintEnd);
			}
			else 
			{
	...
	public:
		void post_unpack() override
		{
			...
			dvars::g_glideOnInspect = dvars::register_bool("g_glideOnInspect", true,
				dvar_flags, "Enables glides on the inspect animation");

			dvars::g_legacyNacs = dvars::register_bool("g_legacyNacs", true,
